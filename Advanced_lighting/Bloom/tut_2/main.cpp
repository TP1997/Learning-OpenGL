...
//Ping-pong framebuffer for blurring.
    ...
    for(unsigned int n=0; n<2; n++){
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        //Attach texture to framebuffer.
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, hdrColorBuffer[1], 0);//hdrColorBuffer[1]
...
/**...                                      */
    ...
    //1. Render scene to floating-point-framebuffer (hdrFBO).
        ...
        for(unsigned int n=1; n<amount; n++){
            glBindFramebuffer(GL_FRAMEBUFFER, pingpongFBO[horizontal]);
            gaussianBlurShader.setBool("horizontal", horizontal);
            glBindTexture(GL_TEXTURE_2D, hdrColorBuffer[1]);//hdrColorBuffer[1]
            renderQuad(quadVAO);
            horizontal=!horizontal;
        }
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
